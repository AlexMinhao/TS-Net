...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        df_stamp = df_raw[['date']][border1:border2]  #(8640, 5)
    ...
    def __getitem__(self, index):
        s_begin = index
        s_end = s_begin + self.seq_len  # 0  +  24
        r_begin = s_end - self.label_len  # 24 - 24 = 0
        r_end = r_begin + self.label_len + self.pred_len #  0 + 24 + 24
...
class Dataset_ECL_hour(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='LD2011_2014.txt',
                 target='OT', scale=True):
        # size [seq_len, label_len pred_len]
        # info
        if size == None:
    ...
    def __read_data__(self):
        scaler = StandardScaler()
        df_raw = pd.read_table("F:\school\Papers\\timeseriesNew\Informer-master\data\ECL\ECL.txt", sep =";")
        print('load data')
        border1s = [0, 12 * 30 * 24 - self.seq_len, 12 * 30 * 24 + 4 * 30 * 24 - self.seq_len]  # [0, 8544, 11424]
        border2s = [12 * 30 * 24, 12 * 30 * 24 + 4 * 30 * 24, 12 * 30 * 24 + 8 * 30 * 24]  # [8640, 11520, 14400]
        border1 = border1s[self.set_type]
        ...
        if self.features == 'M':
            cols_data = df_raw.columns[
                        1:]  # Index(['HUFL', 'HULL', 'MUFL', 'MULL', 'LUFL', 'LULL', 'OT'], dtype='object')
            df_data = df_raw[cols_data]  # (17420, 7)
        elif self.features == 'S':
            df_data = df_raw[[self.target]]
...
if __name__ == '__main__':
    ...
    Data = Dataset_ECL_hour
    data_set = Data(
        root_path='F:\\school\\Papers\\timeseriesNew\\Informer-master\\data\\ECL',
        data_path='ECL.txt',
        flag='train',
        size=[24,24,24],
        features='M'
