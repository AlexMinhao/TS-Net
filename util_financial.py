...
def save_model(model, model_name, model_dir, epoch=None):
   ...
   # file_name = os.path.join(model_dir, 'best_'+ model_name +'.pt')
#    with open(file_name, 'wb') as f:
    torch.save(model.state_dict(), 'model/'+model_dir+'.pth')
#        torch.save(model, f)
...
class DataLoaderS(object):
    ...
    def _split(self, train, valid, test):
        ...
        train_set = range(self.P + self.h - 1, train)  # range(170, 31536)
    ...
    def _batchify(self, idx_set, horizon):
        n = len(idx_set)
        X = torch.zeros((n, self.P, self.m))  # torch.Size([31366, 168, 137])
        Y = torch.zeros((n, self.m))  # torch.Size([31366, 137])  torch.Size([31366, 3, 137])
        for i in range(n):
            end = idx_set[i] - self.h + 1  # 168
            start = end - self.P
            X[i, :, :] = torch.from_numpy(self.dat[start:end, :])
            Y[i, :] = torch.from_numpy(self.dat[idx_set[i], :])
            # Y[i, :] = torch.from_numpy(self.dat[idx_set[i]:self.h, :])
        return [X, Y]
